 ...
 *
 * Provides persistent SSH workspaces for Pi through explicitly named remote_* tools on
 * a verified remote host. Native tools and user shell commands stay local. It manages endpoint configuration, in-memory
...
interface CredentialCache {
  ...
  resume?: { command: string; cwd: string; forwards?: string[] };
}
...
export default function sshRemoteExtension(pi: ExtensionAPI) {
  ...
  const requireRemote = (): RemoteState => {
    if (!remote) throw new Error("SSH remote is disconnected. Connect using remote action=connect or /remote first.");
    return remote;
  };
  ...
  const remotePath = (path: string): string => {
    ...
    if (normalized === "~" || normalized.startsWith("~/")) {
      throw new Error("Use an absolute remote path or a path relative to the remote cwd; ~ is not expanded locally.");
    }
    return posix.resolve(state.cwd, normalized);
  };
  ...
  const limitRemoteToolResult = (result: any, kind: "read" | "exec", startLine = 1, requestedMaxLines?: number) => {
    ...
    if (truncation.firstLineExceedsLimit) {
      text = `[Line ${startLine} exceeds the ${formatSize(maxBytes)} remote read limit. Use remote_bash with sed/head -c to inspect a bounded fragment.]`;
  ...
  const status = (ctx: any) => {
    ...
    else ctx.ui.setStatus("ssh-remote", ctx.ui.theme.fg("accent", `remote_* ${endpointDisplayLabel(remote)}:${remote.cwd}${forwardServers.size ? ` | tunnels ${[...forwardServers.keys()].join(",")}` : ""}`));
  };
  ...
  async function reconnectRemote(): Promise<RemoteState> {
    ...
    reconnectPromise = (async () => {
      ...
      remote = next;
      credentialCache.resume = { command: parsed.command, cwd: next.cwd, forwards: credentialCache.resume?.forwards };
      oldClient?.end();
      if (currentCtx) {
        status(currentCtx);
  ...
  const changeRemoteCwd = async (requested: string, ctx: any): Promise<string> => {
    ...
    remote.cwd = resolved;
    credentialCache.resume = { command: remote.command, cwd: resolved, forwards: [...forwardSpecs.values()].map(serializeForward) };
  ...
  const connectInteractive = async (command: string, ctx: any, cwd?: string): Promise<RemoteState | null> => {
    ...
    try {
      ...
      credentialCache.resume = { command, cwd: next.cwd, forwards: [] };
      lastCommand = command;
      lastConnectionError = undefined;
  ...
  const restoreSessionRemoteState = async (saved: SessionRemoteState, ctx: any): Promise<void> => {
    ...
    credentialCache.resume = { command, cwd: next.cwd, forwards: [...forwardSpecs.values()].map(serializeForward) };
    status(ctx);
  };
  ...
  const disconnect = (ctx: any, forgetCredentials = false) => {
    ...
    status(ctx);
    persistSessionRemoteState();
    ctx.ui.notify(forgetCredentials ? "SSH remote disconnected and cached credentials cleared" : "SSH remote disconnected (credentials remain cached in memory only)", "info");
  };
  ...
  const executeRemoteRead = async (id: string, params: any, signal: AbortSignal | undefined, update: any) => {
    ...
    if (mime && ["image/jpeg", "image/png", "image/gif", "image/webp"].includes(mime)) {
      const result = await createReadTool(requireRemote().cwd, { operations: remoteReadOps() }).execute(id, { ...params, path }, signal, update);
  ...
  pi.registerTool({
    ...
    name: "remote_read",
    label: "Remote Read",
    description: `Read a file on the connected SSH server. Relative paths use the remote cwd; absolute paths stay remote. Text output defaults to ${DEFAULT_READ_MAX_LINES} lines or ${formatSize(DEFAULT_READ_MAX_BYTES)}; use offset/limit to continue. Requires a connection; never falls back to local files.`,
    promptSnippet: "Read files on the connected SSH server",
    promptGuidelines: ["Use remote_read with offset/limit for remote code and logs."],
    execute: (id, params, signal, update) => executeRemoteRead(id, params, signal, update),
  });
  pi.registerTool({
    ...createWriteTool(localCwd),
    name: "remote_write",
    label: "Remote Write",
    description: "Write a file on the connected SSH server, creating parent directories. Relative paths use the remote cwd; absolute paths stay remote. Requires a connection; never writes locally.",
    promptSnippet: "Write files on the connected SSH server",
    promptGuidelines: ["Use remote_write only for files on the connected SSH server."],
    execute: (id, params, signal, update) => {
      const path = remotePath(params.path);
      return createWriteTool(requireRemote().cwd, { operations: remoteWriteOps() }).execute(id, { ...params, path }, signal, update);
    },
  });
  pi.registerTool({
    ...
    name: "remote_edit",
    label: "Remote Edit",
    description: "Edit a file on the connected SSH server using exact text replacement. Relative paths use the remote cwd; absolute paths stay remote. Requires a connection; never edits locally.",
    promptSnippet: "Edit files on the connected SSH server",
    promptGuidelines: ["Use remote_edit only for files on the connected SSH server."],
    execute: async (id, params, signal, update) => {
      const path = remotePath(params.path);
      const result = await createEditTool(requireRemote().cwd, { operations: remoteEditOps() }).execute(id, { ...params, path }, signal, update);
      return limitRemoteToolResult(result, "read");
    },
  });
  pi.registerTool({
    ...
    name: "remote_bash",
    label: "Remote Bash",
    description: `Execute a shell command in the connected SSH server's cwd. Requires a connection; never executes locally. Default timeout: ${DEFAULT_REMOTE_TIMEOUT_SECONDS}s. Output defaults to the last ${DEFAULT_EXEC_MAX_LINES} lines or ${formatSize(DEFAULT_EXEC_MAX_BYTES)}; complete oversized output is saved to a local temporary file, accessible with local read.`,
    promptSnippet: "Run shell commands on the connected SSH server",
    promptGuidelines: ["Use remote_bash for remote commands and bounded log searches. Use remote action=chdir for persistent remote directory changes."],
    execute: async (id, params, signal, update) => {
      const result = await createBashTool(requireRemote().cwd, { operations: remoteBashOps() }).execute(id, params, signal, update);
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _update, ctx) {
      if (params.action === "status") {
        ...
        const text = `${remote ? `Connected: ${endpointDisplayLabel(remote)}:${remote.cwd}; remote_* target: SSH; local tools unchanged` : "SSH remote is disconnected"}${mappings.length ? `; forwarded local ports: ${mappings.join(", ")}` : ""}`;
        return { content: [{ type: "text", text }], details: { connected: Boolean(remote), cwd: remote?.cwd, toolRouting: "explicit", remoteTools: Boolean(remote), forwardedPorts: mappings } };
      }
      if (params.action === "disconnect" || params.action === "forget") {
        disconnect(ctx, params.action === "forget");
        return { content: [{ type: "text", text: params.action === "forget" ? "Disconnected and forgot the cached credentials." : "Disconnected from SSH remote. Local tools are unchanged." }], details: { connected: false } };
      }
      ...
      if (params.action === "forward") {
        ...
        for (const spec of specs) await startForward(spec);
        credentialCache.resume = { command: state.command, cwd: state.cwd, forwards: [...forwardSpecs.values()].map(serializeForward) };
        if (currentCtx) status(currentCtx);
        persistSessionRemoteState();
        const ports = specs.map((spec) => spec.localPort);
        return { content: [{ type: "text", text: `Forwarded local ports: ${ports.join(", ")}; remote_* still targets SSH and local tools stay local.` }], details: { toolRouting: "explicit", forwardedPorts: ports } };
      }
    ...
    handler: async (args, ctx) => {
      ...
      if (/^forward(?:\s+|$)/i.test(input)) {
        try {
          ...
          status(ctx);
          persistSessionRemoteState();
          ctx.ui.notify(`SSH remote port forwarding started; remote_* still targets SSH, local tools stay in ${localCwd}: ${specs.map((spec) => `127.0.0.1:${spec.localPort}`).join(", ")}`, "info");
  ...
  pi.on("before_agent_start", (event) => remote ? {
